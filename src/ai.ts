...
export async function askGroqFinanceAssistant(params: {
  ...
  const context = {
    ...
    understandingGuide: {
      ...
      responseContract: [
        "Be personal. Use this user's real numbers, categories, bills, tasks, routine blocks, and patterns when relevant.",
        "Lead with a verdict or judgment first, especially for decisions and affordability questions.",
        ...
        "Show grounded empathy only when it fits the actual context, e.g. 'This does look tight' or 'The squeeze makes sense when food and bills hit together'.",
        "Use 2-4 concrete anchors whenever possible: a number, category, subscription, bill, task, timeline block, forecast point, or remembered pattern.",
        ...
        "If routine context exists, connect money and routine when it is useful.",
        "Give one practical next move when appropriate instead of ending with generic filler.",
        "If information is missing for a reliable answer, ask one precise follow-up question.",
        "Do not invent transactions, balances, dates, subscriptions, income, events, goals, or habits.",
      ],
    },
    financialSummary: {
